...
<div class="admin-page-content bg-white rounded-2xl shadow-3d p-6 min-w-0 max-w-4xl">
    <form method="POST" action="{{ route('admin.payment-methods.update', $paymentMethod->id) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                >
                    ...
                    <option value="upi" {{ old('type', $paymentMethod->type) === 'upi' ? 'selected' : '' }}>UPI</option>
                    <option value="bank" {{ old('type', $paymentMethod->type) === 'bank' ? 'selected' : '' }}>Bank Transfer</option>
                    <option value="razorpay" {{ old('type', $paymentMethod->type) === 'razorpay' ? 'selected' : '' }}>Razorpay</option>
        ...
        <div id="bank_fields" class="grid grid-cols-1 md:grid-cols-2 gap-4" style="display: {{ old('type', $paymentMethod->type) === 'bank' ? 'grid' : 'none' }};">
            <div>
            ...
            <div>
                ...
                <input 
                    type="text" 
                    name="ifsc_code" 
                    value="{{ old('ifsc_code', $paymentMethod->ifsc_code) }}" 
                    class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" 
            ...
            <div>
                ...
                >
                    ...
                    <option value="savings" {{ old('account_type', $paymentMethod->account_type) === 'savings' ? 'selected' : '' }}>Savings</option>
                    <option value="current" {{ old('account_type', $paymentMethod->account_type) === 'current' ? 'selected' : '' }}>Current</option>
                    <option value="other" {{ old('account_type', $paymentMethod->account_type) === 'other' ? 'selected' : '' }}>Other</option>
                </select>
                @error('account_type')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
        ...
        <div id="upi_fields" style="display: {{ old('type', $paymentMethod->type) === 'upi' ? 'block' : 'none' }};">
            ...
            >{{ old('upi_ids_text', is_array($paymentMethod->upi_ids) ? implode("\n", $paymentMethod->upi_ids) : '') }}</textarea>
        ...
        <div>
            ...
            @if($paymentMethod->scanner_photo)
                <div class="mb-2">
                    <img src="{{ asset('storage/' . $paymentMethod->scanner_photo) }}" alt="QR Code" class="max-w-xs h-auto border rounded-lg">
            ...
            <input 
                ...
                class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" 
            >
            <p class="text-xs text-gray-500 mt-1">Upload QR code image (JPG, PNG, max 5MB). Leave empty to keep current image.</p>
            @error('scanner_photo')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
